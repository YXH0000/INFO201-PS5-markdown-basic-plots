...
# Rank the countries in each year based on life expectancy
ranked_data <- gapminder %>%
  filter(time %in% c(1960, 2019)) %>%
  group_by(time) %>%
  mutate(rank_life_expectancy = rank(desc(lifeExpectancy)))

# Extract the rank for the US in each year
us_ranks <- ranked_data %>%
  filter(region == "United States of America") %>%
  select(time, rank_life_expectancy)
