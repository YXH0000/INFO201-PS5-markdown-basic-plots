...
gdp_1960 <- gapminder %>% filter(time == 1960 & !is.na(GDP_PC))
gdp_2019 <- gapminder %>% filter(time == 2019 & !is.na(GDP_PC))

# Create the histograms and combine them using ggplot2
hist_1960 <- ggplot(gdp_1960, aes(x = GDP_PC)) +
  geom_histogram(binwidth = 1000, fill = "lightblue", color = "black") +
  labs(title = "GDP per Capita in 1960", x = "GDP per Capita", y = "Count") +
  theme_minimal()

hist_2019 <- ggplot(gdp_2019, aes(x = GDP_PC)) +
  geom_histogram(binwidth = 1000, fill = "lightgreen", color = "black") +
  labs(title = "GDP per Capita in 2019", x = "GDP per Capita", y = "Count") +
  theme_minimal()

# Combine histograms
combined_hist <- ggplot() +
  # Add first histogram
  annotation_custom(ggplotGrob(hist_1960), xmin=-Inf, xmax=0.5, ymin=-Inf, ymax=1) +
  # Add second histogram
  annotation_custom(ggplotGrob(hist_2019), xmin=0.5, xmax=Inf, ymin=-Inf, ymax=1) +
  # Add separator line
  geom_vline(xintercept = 0.5, color = "black") +
  # Set plot title and axis labels
  labs(title = "Combined Histogram of GDP Per Capita (1960 vs 2019)",
       x = "GDP Per Capita (constant 2010 US$)",
       y = "Frequency") +
  # Set plot theme
  theme_minimal()
